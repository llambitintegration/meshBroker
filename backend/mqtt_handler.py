...
from message_processor import MessageProcessor, FilterChain, TopicFilter, FilterAction
from message_queue import MessageQueue, FlowController
from message_delivery import MessageDeliveryTracker, QoSHandler
from broker_monitor import MQTTBrokerMonitor
...
class MQTTHandler:
    ...
    def _publish_direct(self, topic: str, payload: Any, qos: int, retain: bool, message_id: int = None) -> Any:
        ...
        try:
            # Apply QoS handling
            msg_id, prepared_payload = self.qos_handler.prepare_publish(topic, payload, qos, retain)
            
            # Actually send the message
            info = self.client.publish(topic, prepared_payload, qos, retain)
            
            if info.rc != mqtt.MQTT_ERR_SUCCESS:
                logger.error(f"Failed to publish message to {topic}: {mqtt.error_string(info.rc)}")
                return None
            else:
    ...
    def _retry_publish(self, topic: str, payload: Any, qos: int, retain: bool, message_id: int):
        ...
        try:
            ...
            info = self.client.publish(topic, payload, qos, retain, message_id)
        ...
        try:
            ...
            if self.message_callback:
                ...
                asyncio.run_coroutine_threadsafe(
                    self.message_callback(processed_topic, processed_payload),
    ...
    def _process_queued_message(self, message):
        ...
        try:
            ...
            # Store message if publishing failed and persistence is enabled
            if (not result or result.rc != mqtt.MQTT_ERR_SUCCESS) and self.persistence_enabled and self.message_store:
                self.message_store.store_message(
                    message_id=message.id,
                    topic=message.topic,
                    payload=message.payload,
                ...
                logger.info(f"Queued message {message.id} stored for later delivery")
        except Exception as e:
            logger.error(f"Error processing queued message {message.id}: {e}")
    
    def _on_flow_control_change(self, throttling: bool):
        """Handle flow control state changes"""
        if throttling:
            logger.warning("Flow control activated, QoS 0 messages will be dropped")
        else:
            logger.info("Flow control deactivated, resuming normal operation")
